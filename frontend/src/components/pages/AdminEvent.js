...
import {startCreateEvent, changeEventState } from '../../services/AdminEvents/actions';
...
export class AdminEventRaw extends Component {
  ...
  placeRequired() {
    ...
    let numEmpt = this.props.num;
     if (cityEmpt === "" || streetEmpt === "" || numEmpt === "") {
         alert("Vyplňte místo konání akce! Povinná pole jsou: město, ulice a číslo popisné.");
         return false;
      }
    return true;
  }
  ...
  submitEvent(){
    if(this.eventNameDescrRequired() && this.datetimeRequired()
        && this.placeRequired() && this.priceRequired())
    {
...
export const AdminEvent = connect(mapStateToProps, mapDispatchToProps)(AdminEventRaw);
