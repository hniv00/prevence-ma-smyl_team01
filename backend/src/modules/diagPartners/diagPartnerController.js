...
export const deleteDiagPartnerController = async (req, res) => {
    ...
    db.Diag_Partner.destroy({
        ...
        .then(deletedDiagPartner => {
            res.json({
                response: deletedDiagPartner == 1 ? `Relation with ID ${req.params.id} was deleted` : `Relation with ID ${req.params.id} was not deleted`
            })
        });
};
...
export const updateDiagPartnerController = async (req, res) => {
    ...
    db.Diag_Partner.update({
        ...
        .then(result => { return res.json("Relation updated"); })
        .catch(err => { return res.json("An error occured while updating the relation"); })
};
