...
export const deleteArticleController = async (req, res) => {
    ...
    db.Article.destroy({
        ...
        .then(deletedArticle => {
            res.json({
                response: deletedArticle == 1 ? `Article with ID ${req.params.id} was deleted` : `Article with ID ${req.params.id} was not deleted`
            })
        });
};
...
export const articleController = async (req, res) => {
    ...
    const articleList = await db.Article.findAll({
        ...
        attributes: ['IDArticle', 'ArticleName', 'ArticleText', 'Photo', 'DiagnosisID', 'ExaminationID'],
        raw: true,
    });
...
export const updateArticleController = async (req, res) => {
    ...
    db.Article.update({
        ...
        .then(result => { return res.json("Article updated"); })
        .catch(err => { return res.json("An error occured while changing the article"); })
};
