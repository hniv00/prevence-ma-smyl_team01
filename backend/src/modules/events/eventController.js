...
export const deleteEventController = async (req, res) => {
    ...
    db.Event.destroy({
        ...
        .then(deletedEvent => {
            res.json({
                response: deletedEvent == 1 ? `Event with ID ${req.params.id} was deleted` : `Event with ID ${req.params.id} was not deleted`
            })
        });
};
...
export const updateEventController = async (req, res) => {
    ...
    db.Event.update({
        ...
        .then(result => { return res.json("Event updated"); })
        .catch(err => { return res.json("An error occured while changing the event"); })
};
