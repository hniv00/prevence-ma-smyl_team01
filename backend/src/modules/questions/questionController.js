...
export const deleteQuestionController = async (req, res) => {
    ...
    db.Question.destroy({
        ...
        .then(deletedQuestion => {
            res.json({
                response: deletedQuestion == 1 ? `Question with ID ${req.params.id} was deleted` : `Question with ID ${req.params.id} was not deleted`
            })
        });
};
...
export const updateQuestionController = async (req, res) => {
    ...
    db.Question.update({
        ...
        .then(result => { return res.json("Question updated"); })
        .catch(err => { return res.json("An error occured while updating the question"); })
};
